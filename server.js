...
MongoClient.connect('mongodb://localhost:27017/jewellery', (err,database) =>{        
	if(err) return console.log(err)
	db=database.db('jewellery')     
	app.listen(5000,() =>{
...
//homepage
app.get('/', (req,res)=>{                
	db.collection('chains').find().toArray( (err,result)=>{     
		if(err) return console.log(err)     
...
app.post('/AddData', (req,res)=>{			
	db.collection('chains').save(req.body, (err,result)=>{			
		if(err) return console.log(err)
	res.redirect('/')                        
	})
})
...
app.post('/update', (req,res)=>{			
		db.collection('chains').find().toArray((err,result)=>{	   			
			...
			db.collection('chains').findOneAndUpdate({pid:req.body.id},{
...
app.post('/delete',(req,res)=>{
	db.collection('chains').findOneAndDelete({pid:req.body.id}, (err,result)=>{    
		if(err)
			return console.log(err)
		res.redirect('/')
	})
